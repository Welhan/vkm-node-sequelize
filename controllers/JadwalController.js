...
module.exports = {
  ...
  index: async function (req, res) {
    ...
    let akses = await helpers.checkUserAccess(UserID, 2, 0);
    ...
    let fitur = await helpers.checkFiturAccess(UserID, 2, 0);
    return res.render("jadwal/index", {
  ...
  getDataJadwal: async function (req, res) {
    if (!req.session.ID) {
    ...
    let getDataJadwal = await jadwalModel.getForTeacher(username);
    let fitur = await helpers.checkFiturAccess(UserID, 2, 0);
    return res.json({
      data: getDataJadwal,
      fitur:
  ...
  getAddJadwal: async function (req, res) {
    if (!req.session.ID) {
      return res.status(200).json({ logout: "/logout" });
    }

    let user = await helpers.checkActiveUser(req.session.ID);
    ...
    let murid;
    if (user.Role == "Guru") {
      murid = await jadwalModel.getMuridNotExist(req.session.UserID);
    } else {
      murid = await jadwalModel.getMuridNotExist();
    }
  ...
  getBidangMurid: async function (req, res) {
    if (!req.session.ID) {
      return res.status(200).json({ logout: "/logout" });
    }

    let murid = req.body.murid;
    if (!murid) return res.status(200).json({ data: [] });
    let dataMurid = await muridModel.getByName(murid);

    let listGuru = dataMurid.GuruID.split(",");
    let listBidang = dataMurid.Les.split(",");

    const guruIndex = [];
    const bidangGuru = [];
    if (listGuru.length > 0) {
      listGuru.forEach((guru, idx) => {
        if (guru.trim() === req.session.UserID) {
          guruIndex.push(idx);
        }
      });
      bidangGuru = guruIndex
        .map((idx) => listBidang[idx]?.trim())
        .filter(Boolean);
    }

    return res.status(200).json({ data: bidangGuru });
  },
  newJadwal: async function (req, res) {
    if (!req.session.ID) {
      return res.redirect("/");
    }

    let murid = req.body.murid;
    let hari = [].concat(req.body.hari || []);
    let bidang = [].concat(req.body.bidang || []);
    let trial = [].concat(req.body.trial || []);
    trial = hari.map((_, i) => (trial[i] === "1" ? 1 : 0));
    let jam = []
      .concat(req.body.jam)
      .map((j) => (j === "" || j == null ? "10:00" : j));
    let error = {};
    ...
    let namaGuru = await adminModel.getByUsername(req.session.UserID);
    let guruName = namaGuru.NamaDepan + " " + namaGuru.NamaBelakang;
    ...
    for (let i = 0; i < hari.length; i++) {
      ...
      try {
        const insertId = await jadwalModel.create(data);
        if (insertId) {
          req.flash("success", `Jadwal ${murid} berhasil ditambahkan!`);
        } else {
          req.flash("error", `Jadwal ${murid} gagal ditambahkan!`);
        }
      } catch (error) {
        console.error(`Gagal saat insert data ke-${i + 1}:`, error);
        req.flash("error", `Terjadi kesalahan saat insert ke-${i + 1}`);
      }
    }
    ...
    try {
      let insertId = await adminModel.create({
        name: nama,
        name2: name2,
        telp: telp,
        tglLahir: tglLahir,
  ...
  getEditJadwal: async function (req, res) {
    if (!req.session.ID) {
      return res.status(200).json({ logout: "/logout" });
    }
    ...
    let dataGuru = activeUser.NamaDepan + " " + activeUser.NamaBelakang;

    if (guru != dataGuru.trim()) {
      return res.status(200).json({ redirect: "/logout" });
    }

    let data = await jadwalModel.getForTeacher(guru, true);
    res.render(
      "jadwal/modals/editJadwal",
      {
        layout: false,
        csrfToken: req.csrfToken(),
        data,
